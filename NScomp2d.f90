...
subroutine initialize(x,y,dx,dy,xmax,ymax,rho, u, v, T,nx,ny,imax,jmax)
    ...
    do j=-2,jmax,1;y(j)=(j-jmax/2)*dy; end do
...
subroutine update(rho, u, v, T, p, E, rho_new, u_new, v_new, T_new,re,pr,gamma,mach,&
    ...
    do j = 2, ny-1
        do i = 2, nx-1
            ! Update equations
            rho_new(i,j) = rho(i,j)+dt*((fr(i+1,j)-fr(i-1,j))/2./dx+(gr(i,j+1)-gr(i,j-0))/1./dy)
            ru_new=rho(i,j)*u(i,j) +dt*((fu(i+1,j)-fu(i-1,j))/2./dx+(gu(i,j+1)-gu(i,j-0))/1./dy)
            rv_new=rho(i,j)*v(i,j) +dt*((fv(i+1,j)-fv(i-1,j))/2./dx+(gv(i,j+1)-gv(i,j-0))/1./dy)
            e_new        =   E(i,j)+dt*((fe(i+1,j)-fe(i-1,j))/2./dx+(ge(i,j+1)-ge(i,j-0))/1./dy)
...
do i = 1, nx
    dudy(i,1) = (u(i,j+1) - u(i,j)) / (1.0*dy)
    dvdy(i,1) = (v(i,j+1) - v(i,j)) / (1.0*dy)
    dTdy(i,1) = (T(i,j+1) - T(i,j)) / (1.0*dy)
    dudy(i,ny) = (u(i,j) - u(i,j-1)) / (1.0*dy)
    dvdy(i,ny) = (v(i,j) - v(i,j-1)) / (1.0*dy)
    dTdy(i,ny) = (T(i,j) - T(i,j-1)) / (1.0*dy)
end do 
do j = 1, ny
  do i = 1, nx
...
call viscous_flux(rho, u, v, T, p, E, fuvis,fvvis,fevis,guvis,gvvis,gevis, &
 ...
 do j = 2, ny-1
    do i = 2, nx-1
! Update equations
        rr_new=A54(ir)*rho_new(i,j)+dt*((fr(i+1,j)-fr(i-1,j))/2./dx+(gr(i,j+1)-gr(i,j-0))/1./dy)
        ru_new=A54(ir)*rho_new(i,j)*u(i,j) +dt*((fu(i+1,j)-fu(i-1,j))/2./dx+(gu(i,j+1)-gu(i,j-0))/1./dy)
        rv_new=A54(ir)*rho_new(i,j)*v_new(i,j) +dt*((fv(i+1,j)-fv(i-1,j))/2./dx+(gv(i,j+1)-gv(i,j-0))/1./dy)
        rE_new=A54(ir)*E_new(i,j)+dt*((fe(i+1,j)-fe(i-1,j))/2./dx+(ge(i,j+1)-ge(i,j-0))/1./dy)
 
        rho(i,j)=B54(ir) * rr_new + rho(i,j)
        u(i,j)  =B54(ir) * ru_new+ rho(i,j)*u(i,j)
        v(i,j)  =B54(ir) * rv_new+ rho(i,j)*v(i,j)
        E(i,j)  =B54(ir) * rE_new + E(i,j)

        u(i,j)=u(i,j)/rho(i,j)
        v(i,j)=v(i,j)/rho(i,j)
        p(i,j)=(E(i,j)-0.5*(u(i,j)**2+v(i,j)**2))*(gamma-1)
        T(i,j)=p(i,j)/rho(i,j)/gamma/mach**2

        !write(12,'(2I4,6F16.10)'),i,j,u(i,j),v(i,j),u_new(i,j),v_new(i,j),ru,ru_new
        write(12,'(2I4,6F16.10)'),i,j,rho(i,j),u(i,j),v(i,j),E(i,j),T(i,j),p(i,j)
        write(12,'(2I4,6F16.10)'),i,j,rho_new(i,j),u_new(i,j),v_new(i,j),E_new(i,j),T_new(i,j),p_new
    end do
  end do
  call apply_bc(rho, u, v, T,nx,ny,imax,jmax)
end do
